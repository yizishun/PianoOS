...
impl<C> ArchTime for Riscv64<C> {
	default fn sleep(&self, sec: usize) {
		let time_freq = PLATFORM.get().unwrap().board_info.cpu_freq.unwrap();
		let time_start = time::read();
		let time_end = time_start + (time_freq * sec) as usize;
		unsafe {
			sie::set_stimer();
		}
	...
	default fn time_ns(&self) -> u64 {
		let time_freq = PLATFORM.get().unwrap().board_info.cpu_freq.unwrap();
		let time = time::read64();
		(time * 1_000_000_000) / time_freq as u64
	}

	default fn time_us(&self) -> u64 {
		let time_freq = PLATFORM.get().unwrap().board_info.cpu_freq.unwrap();
		let time = time::read64();
		(time * 1_000_000) / time_freq as u64
	}

	default fn time_ms(&self) -> u64 {
		let time_freq = PLATFORM.get().unwrap().board_info.cpu_freq.unwrap();
		let time = time::read64();
		(time * 1_000) / time_freq as u64
	}

	default fn time_s(&self) -> u64 {
		let time_freq = PLATFORM.get().unwrap().board_info.cpu_freq.unwrap();
		let time = time::read64();
		time / time_freq as u64
	}

	default fn set_next_timer_intr(&self, dur_ms: usize) {
		let time_freq = PLATFORM.get().unwrap().board_info.cpu_freq.unwrap();
		let now = time::read64();
		let total_clock_dur = (time_freq as u64 / 1_000) * dur_ms as u64;
